...
angular.module('controllers').controller('manageController', function ($scope, $http, $state, $rootScope) {
    ...
    $scope.edit = false;
    $scope.changed = false;
    $scope.toggleShowInput = function (input) {
        $scope['show_' + input + '_input'] = !$scope['show_' + input + '_input']
    };
    ...
    $scope.submit = function () {
        ...
        $http.put('/auth', $scope.user).success(function () {
            $scope.edit = false;
    ...
    $scope.submitGroup = function () {
        ...
        if (group._id) {
            $http.put('/api/permissionGroups', group).success(function (data) {
                updateGroups();
            });
        } else {
            $http.post('/api/permissionGroups', group).success(function (data) {
                updateGroups();
            });
        }
    };
    $scope.deleteGroup = function () {
        var group = $scope.selectedGroup;
        if (group._id) {
            $http.delete('/api/permissionGroups/' + group._id).success(function (data) {
                updateGroups();
                $scope.newGroup();
            });
        }
