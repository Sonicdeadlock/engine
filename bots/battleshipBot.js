...
function sendStatus(chatFunction, room) {
    switch (room.players.length) {
        ...
        case 1:
            ...
            break;
        case 2: {
            var unreadyPlayers = _.filter(room.players, function (o) {
                return !o.ready
            });
        ...
        player.ships.forEach(function (ship) {
            if (_.inRange(x, ship.startX, ship.endX + 1) && _.inRange(y, ship.startY, ship.endY + 1)) {
                ...
                if (ship.hits == ship.size) {
                    ...
                    ship.sunk = true;
                    var hasUnsunkShip = _(player.ships).map('sunk').reduce(function (hasFoundUnsunk, currentIsSunk) {
                        return hasFoundUnsunk || !currentIsSunk
                    }, true).value();
                    if (!hasUnsunkShip) {
